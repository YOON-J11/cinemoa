document.addEventListener("DOMContentLoaded", function () {
  ...
  // 서버에서 Mustache 템플릿으로 받아온 변수들 (템플릿에 선언되어 있어야 함)
  ...
  // 다음 버튼 눌렀을 때 로그인 확인 후 결제 페이지로 이동
    ...
    fetch("/checkLogin", {
      ...
      .then(data => {
        if (data.loggedIn) {
          ...
          // 좌석 ID 리스트
          const seatIdList = Array.from(selectedSeatElements).map(el => Number(el.dataset.seatId));

          // 좌석 문자열 ("A1,A2")
          const seatInfo = Array.from(selectedSeatElements).map(el => {
          ...
          // 결제 페이지 URL 생성
          const paymentUrl = `/reservation/payment?amount=${totalPrice}&cinemaId=${cinemaId}&movieId=${movieId}&showtimeId=${showtimeId}&screenId=${screenId}&seatInfo=${encodeURIComponent(seatInfo)}&seatIdList=${encodeURIComponent(JSON.stringify(seatIdList))}`;
